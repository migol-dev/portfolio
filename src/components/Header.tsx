    ...
    >
      ...
      {mobileOpen && (
        <nav
          ...
          className="md:hidden border-t border-border bg-void/95 backdrop-blur-sm px-4 py-4 flex flex-col gap-1.5"
        >
          {TABS.map((tab) => (
            <a
              key={tab.href}
              ...
              className={cn(
                "rounded px-3 py-3 text-sm font-mono",
          ...
          <a
            href="#contact"
            onClick={() => setMobileOpen(false)}
            className="rounded-md border border-cyan/40 bg-cyan/10 px-3.5 py-2.5 text-sm font-mono text-cyan text-center hover:bg-cyan/20 transition-colors mt-2"
          >
            $ contactar
          </a>
        </nav>
