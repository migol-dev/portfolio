...
export function Skills({ skills }: { skills: Skill[] }) {
  return (
    <section id="skills" className="py-16 sm:py-20 lg:py-28 border-t border-border">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <Reveal>
          <SectionEyebrow path="src/skills.ts" label="Habilidades" />
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4">Con qué construyo</h2>
          ...
          {skills.map((skill, i) => {
            ...
            return (
              <Reveal key={skill.id} delay={Math.min(i * 0.05, 0.4)}>
                <article
                  className={cn(
                    "group rounded-lg border border-border bg-panel/60 p-4 sm:p-5",
                ...
                >
                  ...
                  <h3 className="text-sm sm:text-base font-semibold text-text mb-1">{skill.name}</h3>
                  <p className="text-[10px] sm:text-xs font-mono text-text-faint mb-2 uppercase tracking-wide">
                    {CATEGORY_LABELS[skill.category] ?? skill.category}
                  </p>
                  <p className="text-xs sm:text-sm text-text-dim leading-relaxed">{skill.summary}</p>
                </article>
              </Reveal>
            );
          })}
